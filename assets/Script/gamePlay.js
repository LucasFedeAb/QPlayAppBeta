...
const siginOut = document.querySelector("#sigin-out");
...
navGeography.addEventListener("click", function () {
  varValue("Geografia");
});
navHistory.addEventListener("click", function () {
  varValue("Historia");
});
navDevs.addEventListener("click", function () {
  varValue("Programacion");
});
navAll.addEventListener("click", function () {
  varValue("Aleatorio");
});
siginOut.addEventListener("click", function () {
...
function updateMaxScore(score, category) {
  maxScore = JSON.parse(localStorage.getItem("maxScore")) || {
    score: 0,
    category: "",
  }; // obtiene el puntaje máximo del localStorage o lo inicializa a 0 y una categoría vacía si no hay uno guardado
...
function showMaxScore() {
  ...
  if (maxScoreData) {
    console.log(
      `El puntaje máximo es ${maxScoreData.score} en la categoría ${maxScoreData.category}`
    );
  } else {
...
function checkAnswer(selectedAnswer) {
  ...
  if (selectedAnswer === answerCategory[currentQuestion].correctAnswer) {
    ...
    document
      .querySelector(`.btn${answerCategory[currentQuestion].correctAnswer}`)
...
function showQuestion() {
  ...
  showBtn0.addEventListener("click", function () {
    checkAnswer(0);
  });
...
function showStatistics() {
  if (!maxScoreData) {
    ...
    Swal.fire({
      background: "#C9FFA5",
      title: "MEJOR PUNTUACIÓN",
      html: `<strong>El mejor puntaje es ${maxScoreData.score} en la categoría ${maxScoreData.category}</strong>`,
      showCloseButton: true,
      showConfirmButton: false,
      showClass: {
        popup: "animate__animated animate__fadeInDown",
